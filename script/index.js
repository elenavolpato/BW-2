...
const getArtists = function () {
  fetch("https://striveschool-api.herokuapp.com/api/deezer/search?q=artist")
    ...
    .then((data) => {
      ...
      const randomArtists = data.data.sort(() => Math.random() - 0.5).slice(0, 9);
      for (let i = 0; i < randomArtists.length; i++) {
        row.innerHTML += `
          <div class="col-4 mt-2 mb-4"> 
            <input type="checkbox" class="btn-check" value="${randomArtists[i].artist.id}" id="btn-check-${i}" autocomplete="off" />
            <label class="btn" for="btn-check-${i}">
              <img src="${randomArtists[i].artist.picture_medium}" class="card-img-top rounded-circle p-3" />
              <span class="text-white small">${randomArtists[i].artist.name}</span>
            </label>
          </div>`;
      }
